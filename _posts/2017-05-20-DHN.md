...
![@之前的损失函数](../images/deephash/DHN-12.jpg) instead of the pairwise cross-entropy loss.
![@目标函数的不同组合的结果实验结果](../images/deephash/DHN-Table2.jpg)
We investigate several variants of DHN: DHN-B is the DHN
variant without binarization (h ← sgn(zl) not performed), which may serve as an upper bound of performance. DHNQ is the DHN variant without the quantization loss (λ = 0). DHN-E is the DHN variant using widely-adopted pairwise quared loss.
