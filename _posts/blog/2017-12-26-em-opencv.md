...
$$\sum_i \log p(x^{(i)}; \theta) = \sum_i \log \sum_{z^{(i)}} p (x^{(i)}, z^{(i)}; \theta)     \qquad (1) $$  
 $$ =  \sum_i \log \sum_{z^{(i)}} Q_i(z^{(i)})\frac{p (x^{(i)}, z^{(i)}; \theta) }{Q_i(z^{(i)})}   \qquad (2)$$ 
 $$ \ge  \sum_i  \sum_{z^{(i)}} Q_i(z^{(i)})\log\frac{p (x^{(i)}, z^{(i)}; \theta) }{Q_i(z^{(i)})}   \qquad(3)$$
 本质上我们是需要最大化（1）式（*对（1）式，我们回忆下**联合概率密度**下某个变量的边缘概率密度函数的求解，注意这里$z$也是随机变量。对每一个样本$i$的所有可能类别$z$求等式右边的联合概率密度函数和，也就得到等式左边为随机变量x的边缘概率密度*），也就是似然函数，但是可以看到里面有“和的对数”，求导后形式会非常复杂（自己可以想象下log(f1(x)+ f2(x)+ f3(x)+…)复合函数的求导），所以很难求解得到未知参数$z$和$θ$。
