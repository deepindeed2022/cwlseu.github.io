...
$ W_{t+1} = arg min_W \{\frac{1}{t}\sum_{r=1}^{t} G^r +  \lambda\|W\|_1 +  \frac{\gamma}{2\sqrt t} \|W\|^2_2\}$ 公式（3-2）
直接求解上式看上去非常困难，但是我们可以仿照上一篇FOBOS中采用的方法，针对特征权重的各个维度将其拆解成N个独立的标量最小化问题：

$ w_{t+1} = arg min_{w_i \in R} \{\overline{g}_i^t +  \lambda|w| +  \frac{\gamma}{2\sqrt t} w_i^2\}$     公式（3-3）
